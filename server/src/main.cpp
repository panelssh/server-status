...
CConfig::CConfig()
{
  ...
  str_copy(m_aJSONFile, "json/status.json", sizeof(m_aJSONFile));
...
void CMain::OnNewClient(int ClientNetID, int ClientID)
{
  ...
  if (Client(ClientID)->m_ClientNetType == NETTYPE_IPV4)
    Client(ClientID)->m_status.m_Status4 = true;
  else if (Client(ClientID)->m_ClientNetType == NETTYPE_IPV6)
    Client(ClientID)->m_status.m_Status6 = true;
}
...
void CMain::OnDelClient(int ClientNetID)
{
  ...
  if (ClientID >= 0 && ClientID < NET_MAX_CLIENTS)
  {
    ...
    mem_zero(&Client(ClientID)->m_status, sizeof(CClient::Cstatus));
  }
}
...
int CMain::HandleMessage(int ClientNetID, char *pMessage)
{
  ...
  if (str_comp_num(pMessage, "update", sizeof("update") - 1) == 0)
  {
    ...
    if (rStart["uptime"].type)
      pClient->m_status.m_Uptime = rStart["uptime"].u.integer;
    if (rStart["load"].type)
      pClient->m_status.m_Load = rStart["load"].u.dbl;
    if (rStart["network_rx"].type)
      pClient->m_status.m_NetworkRx = rStart["network_rx"].u.integer;
    if (rStart["network_tx"].type)
      pClient->m_status.m_NetworkTx = rStart["network_tx"].u.integer;
    if (rStart["memory_total"].type)
      pClient->m_status.m_MemTotal = rStart["memory_total"].u.integer;
    if (rStart["memory_used"].type)
      pClient->m_status.m_MemUsed = rStart["memory_used"].u.integer;
    if (rStart["swap_total"].type)
      pClient->m_status.m_SwapTotal = rStart["swap_total"].u.integer;
    if (rStart["swap_used"].type)
      pClient->m_status.m_SwapUsed = rStart["swap_used"].u.integer;
    if (rStart["hdd_total"].type)
      pClient->m_status.m_HDDTotal = rStart["hdd_total"].u.integer;
    if (rStart["hdd_used"].type)
      pClient->m_status.m_HDDUsed = rStart["hdd_used"].u.integer;
    if (rStart["cpu"].type)
      pClient->m_status.m_CPU = rStart["cpu"].u.dbl;
    if (rStart["sshd_service"].type)
      pClient->m_status.m_SSHD_Service = rStart["sshd_service"].u.integer;
    if (rStart["stunnel4_service"].type)
      pClient->m_status.m_Stunnel4_Service = rStart["stunnel4_service"].u.integer;
    if (rStart["openvpn_service"].type)
      pClient->m_status.m_OpenVPN_Service = rStart["openvpn_service"].u.integer;
    if (rStart["dropbear_service"].type)
      pClient->m_status.m_Dropbear_Service = rStart["dropbear_service"].u.integer;
    if (rStart["squid3_service"].type)
      pClient->m_status.m_Squid3_Service = rStart["squid3_service"].u.integer;
    if (rStart["badvpn_service"].type)
      pClient->m_status.m_BadVPN_Service = rStart["badvpn_service"].u.integer;

    if (rStart["status4"].type && pClient->m_ClientNetType == NETTYPE_IPV6)
      pClient->m_status.m_Status4 = rStart["status4"].u.boolean;
    if (rStart["status6"].type && pClient->m_ClientNetType == NETTYPE_IPV4)
      pClient->m_status.m_Status6 = rStart["status6"].u.boolean;
  ...
  else if (str_comp_num(pMessage, "pong", sizeof("pong") - 1) == 0)
  {
    ...
    if (!str_comp(pData, "0") || !str_comp(pData, "off"))
      pClient->m_status.m_Pong = false;
    else if (!str_comp(pData, "1") || !str_comp(pData, "on"))
      pClient->m_status.m_Pong = true;
  ...
  while (gs_Running)
  {
    ...
    for (int i = 0; i < NET_MAX_CLIENTS; i++)
    {
      ...
      if (pClients[i].m_Connected)
      {
        // Uptime
        char aUptime[16];
        int Days = pClients[i].m_status.m_Uptime / 60.0 / 60.0 / 24.0;
        if (Days > 0)
        {
          if (Days > 1)
          ...
          str_format(aUptime, sizeof(aUptime), "%02d:%02d:%02d", (int)(pClients[i].m_status.m_Uptime / 60.0 / 60.0), (int)((pClients[i].m_status.m_Uptime / 60) % 60), (int)((pClients[i].m_status.m_Uptime) % 60));

        str_format(pBuf, sizeof(aFileBuf) - (pBuf - aFileBuf), "{ \"server_id\": \"%s\",  \"nameserver\": \"%s\", \"hostname\": \"%s\", \"location\": \"%s\", \"status4\": %s, \"status6\": %s, \"uptime\": \"%s\", \"load\": %.2f, \"network_rx\": %" PRId64 ", \"network_tx\": %" PRId64 ", \"cpu\": %d, \"memory_total\": %" PRId64 ", \"memory_used\": %" PRId64 ", \"swap_total\": %" PRId64 ", \"swap_used\": %" PRId64 ", \"hdd_total\": %" PRId64 ", \"hdd_used\": %" PRId64 ", \"sshd_service\": %" PRId64 ", \"stunnel4_service\": %" PRId64 ", \"ovpn_service\": %" PRId64 ", \"dropbar_service\": %" PRId64 ", \"squid3_service\": %" PRId64 ", \"badvpn_service\": %" PRId64 ", \"custom\": \"%s\" },\n",
                   pClients[i].m_aServerid, pClients[i].m_aNameserver, pClients[i].m_aHostname, pClients[i].m_aLocation, pClients[i].m_status.m_Status4 ? "true" : "false", pClients[i].m_status.m_Status6 ? "true" : "false", aUptime, pClients[i].m_status.m_Load, pClients[i].m_status.m_NetworkRx, pClients[i].m_status.m_NetworkTx, (int)pClients[i].m_status.m_CPU, pClients[i].m_status.m_MemTotal, pClients[i].m_status.m_MemUsed, pClients[i].m_status.m_SwapTotal, pClients[i].m_status.m_SwapUsed, pClients[i].m_status.m_HDDTotal, pClients[i].m_status.m_HDDUsed, pClients[i].m_status.m_SSHD_Service, pClients[i].m_status.m_Stunnel4_Service, pClients[i].m_status.m_OpenVPN_Service, pClients[i].m_status.m_Dropbear_Service, pClients[i].m_status.m_Squid3_Service, pClients[i].m_status.m_BadVPN_Service, pClients[i].m_aCustom);
        pBuf += strlen(pBuf);
      }
      else
      {
        str_format(pBuf, sizeof(aFileBuf) - (pBuf - aFileBuf), "{ \"server_id\": \"%s\",  \"nameserver\": \"%s\", \"hostname\": \"%s\", \"location\": \"%s\", \"status4\": false, \"status6\": false },\n",
                   pClients[i].m_aServerid, pClients[i].m_aNameserver, pClients[i].m_aHostname, pClients[i].m_aLocation);
        pBuf += strlen(pBuf);
      }
    }
...
int CMain::ReadConfig()
{
  ...
  if (rStart.type == json_array)
  {
    for (unsigned i = 0; i < rStart.u.array.length; i++)
    {
      ...
      str_copy(Client(ID)->m_aServerid, rStart[i]["server_id"].u.string.ptr, sizeof(Client(ID)->m_aServerid));
      str_copy(Client(ID)->m_aNameserver, rStart[i]["nameserver"].u.string.ptr, sizeof(Client(ID)->m_aNameserver));
      ...
      str_copy(Client(ID)->m_aHostname, rStart[i]["hostname"].u.string.ptr, sizeof(Client(ID)->m_aHostname));
      str_copy(Client(ID)->m_aLocation, rStart[i]["location"].u.string.ptr, sizeof(Client(ID)->m_aLocation));
      str_copy(Client(ID)->m_aCustom, rStart[i]["custom"].u.string.ptr, sizeof(Client(ID)->m_aCustom));

      if (m_Config.m_Verbose)
      {
        if (Client(ID)->m_Disabled)
          dbg_msg("main", "[#%d: server_id: \"%s\", nameserver: \"%s\", username: \"%s\", password: \"%s\", hostname: \"%s\", location: \"%s\", custom: \"%s\"]",
                  ID, Client(ID)->m_aServerid, Client(ID)->m_aNameserver, Client(ID)->m_aUsername, Client(ID)->m_aPassword, Client(ID)->m_aHostname, Client(ID)->m_aLocation, Client(ID)->m_aCustom);
        else
          dbg_msg("main", "#%d: server_id: \"%s\", nameserver: \"%s\", username: \"%s\", password: \"%s\", Hostname: \"%s\", location: \"%s\", custom: \"%s\"",
                  ID, Client(ID)->m_aServerid, Client(ID)->m_aNameserver, Client(ID)->m_aUsername, Client(ID)->m_aPassword, Client(ID)->m_aHostname, Client(ID)->m_aLocation, Client(ID)->m_aCustom);
      }
