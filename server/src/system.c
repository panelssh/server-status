...
extern "C"
{
	...
	static NETstatus network_status = {0};
	static MEMstatus memory_status = {0};
	...
	void *mem_alloc_debug(const char *filename, int line, unsigned size, unsigned alignment)
	{
		...
		memory_status.allocated += header->size;
		memory_status.total_allocations++;
		memory_status.active_allocations++;
...
#error not implemented
	...
	*/
		...
		/*if(d < 0)
	{
		...
		network_status.sent_bytes += size;
		network_status.sent_packets++;
		return d;
	}
	...
	int net_udp_recv(NETSOCKET sock, NETADDR *addr, void *data, int maxsize)
	{
		...
		if (bytes > 0)
		{
			...
			network_status.recv_bytes += bytes;
			network_status.recv_packets++;
			return bytes;
		}
		else if (bytes == 0)
			return 0;
	...
	void net_status(NETstatus *status_inout)
	{
		*status_inout = network_status;
	}
