...
def get_service(ps_name):
    ps = subprocess.Popen("ps axf | grep %s | grep -v grep | wc -l" %
                          ps_name, shell=True, stdout=subprocess.PIPE)
    ...
    while 1:
        try:
            ...
            while 1:
                ...
                array['uptime'] = Uptime
                array['load'] = Load
                array['sshd_service'] = get_service('sshd')
                array['stunnel4_service'] = get_service('stunnel4')
                array['openvpn_service'] = get_service('openvpn')
                array['dropbear_service'] = get_service('dropbear')
                array['squid3_service'] = get_service('squid3')
                array['badvpn_service'] = get_service('badvpn-udpgw')
